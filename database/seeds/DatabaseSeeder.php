...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $numberOfUser = (int) $this->command->ask('How many users do you need', 20);

        $this->command->info("Creating {$numberOfUser} users.");

        $users = factory(\App\User::class, $numberOfUser)->create();

        $tweetsRange = $this->command->ask('How many tweets oa single user should get in range ?', '10-20');
        $repliesAndLikeRange = $this->command->ask('How many replies and likes a tweet should get.', '5-30');

        $this->command->warn("Updating Database, Please Hold....");
        ...
        $followCount = $this->command->ask('How many followers should a user get.', '2-9');
